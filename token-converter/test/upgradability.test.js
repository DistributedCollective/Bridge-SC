...
const initialDeploymentFee = 15000;
...
contract("Converter", async (accounts) => {
  before(async function () {
    v1_contract = await deployProxy(ConverterContract_v1, [
      initialDeploymentFee,
    ]);
  });

  describe("Converter Contract should", () => {
    ...
    it("ACCEPT transaction and store values", async () => {
  ...
  describe("Delpoy UPGRADED Contract should", () => {
    ...
    it("ACCEPT transaction and store value + 10, as contrac was updated", async () => {
      // THIS TEST MUST BE UPDATED WITH A REAL FUNCTION OF THE CONTRACT
      // THIS TEST MUST BE UPDATED WITH A REAL FUNCTION OF THE CONTRACT
      ...
      const testPauseVarCurrent = (
        await v2_contract.testPauseVar()
      ).toNumber();
