...
module.exports = class GasServices {
    constructor(logger, config, Web3 = web3) {
        ...
        let etherscanApiBaseUrl = config.sidechain.host.includes('rinkeby')
        ...
        this.gasPriceAvg = new GasPriceAvg(log4js.getLogger('ETH-MAINNET-GasPriceFetAvg'));
    }

    async startGasServices() {
        let etherScanGasPriceService = new Scheduler(gasApiPollingInterval, this.logger, {
            run: () => this.runGasPriceService(),
        });
        let gasPriceAvgService = new Scheduler(avgGasPollingInterval, this.logger, {
            run: () => this.runGasPriceAvg(),
        });
    ...
    async runGasPriceAvg() {
        try {
            globals.currentEthGasPriceAvg = await this.gasPriceAvg.calcAvg(
                avgGasCount,
                globals.currentEthGasBasePrice
            );
