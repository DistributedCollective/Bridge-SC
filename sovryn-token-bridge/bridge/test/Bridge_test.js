...
//// Try and fail to process same tx twice.
            ...
            it('should send to ERC20 SC with useData==0', async function () {
                ...
                console.log("this.mirrorErc777Converter.address: " +this.mirrorErc777Converter.address);
                console.log("this.mirrormockERC20Receiver.address: " + this.mirrormockERC20Receiver.address);
                
                await this.mirrorErc777Converter.setBridgeContract(this.mirrorBridge.address, {from: bridgeManager});
                await this.mirrorBridge.setErc777Converter(this.mirrorErc777Converter.address, {from: bridgeManager})    
                const erc777fromGet = await this.mirrorBridge.getErc777Converter();
                //const erc777fromGet = await  this.mirrorBridge.methods.getErc777Converter().call();
                
                console.log("getErc777Converter : " + erc777fromGet);

                let bufZero = web3.eth.abi.encodeParameter('bytes32', web3.utils.rightPad('0x00', 64))
                console.log("bufZero: " + bufZero);
                ...
                receiverBalance = await tsideToken.balanceOf(this.mirrormockERC20Receiver.address);
                console.log("erc20 sc balance: " + receiverBalance);
                console.log("this.amount: " + this.amount);

                assert.equal(receiverBalance, this.amount);
...
//                 const result = utils.hexaToString(receipt.logs[1].args[8]);
// console.log("receipt.logs[1].args[8] :" + receipt.logs[1].args[8]);
// console.log("result :" + result);
// console.log("mirrormockERC20Receiver.address :" + mirrormockERC20Receiver.address);
// console.log("BytesmirrormockERC20Receiver :" + BytesmirrormockERC20Receiver);
 

//                 assert.equal(result, mirrormockERC20Receiver.address);
            ...
            it('should send directly to erc777converter', async function () {
                ...
                //const BytesmirrormockERC20Receiver = Buffer.from(this.mirrormockERC20Receiver.address);
...
// 
                ...
                assert.equal(receiverBalance, this.amount);
                assert.equal(receiverBalance, 2*this.amount);
    ...
    //         const proxy = await this.project.createProxy(Bridge_v1, {
    //                 initMethod: 'initialize',
    //                 initArgs: [this.multiSig.address, this.fedMultiSig.address, this.allowTokens.address, this.mirrorSideTokenFactory.address, 'r']
    //         });
    //         await this.project.upgradeProxy(proxy.address, Bridge);
    ...
    //         await this.multiSig.submitTransaction(this.bridge.address, 0, data, { from: multiSigOnwerA });
