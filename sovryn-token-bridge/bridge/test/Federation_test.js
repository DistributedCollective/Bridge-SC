...
contract('Federation', async function(accounts) {
    ...
    const fedMember1Pk = '0x6ff46791d809f5a588c1339dc065e38d2079eafa6ff32130a6d5686e0b6b60ea';
    const fedMember2 = accounts[3];
    const fedMember2Pk = '0x7a8ecb002e77733e6db66c27c46cc27f72314dbfe9d4a14742052fb5b8198bda';
    const fedMember3 = accounts[4];
    const fedMember3Pk = '0x94a22cb842f63229c589a96c5d60bed0462672d20785d407e144df63dacef64b';
    ...
    describe('Transactions', async function() {
        ...
        it('voteTransaction should be successful with 1/1 feds require 1', async function() {
            ...
            let transactionWasProcessed = await this.federation.transactionWasProcessed(
                transactionId,
                { from: fedMember1 },
            );
            assert.equal(transactionWasProcessed, true);
        ...
        it('executeTransaction should be successful with 2/2 feds require 1', async function() {
            ...
            const deadline = createTimestamp(2);
            const signatures = [];
            const wallet = new ethers.Wallet(fedMember2Pk);
            const payload = ethers.utils.solidityPack(
                ['bytes32', 'uint256', 'address', 'uint256'],
                [transactionId, 1, this.federation.address, deadline],
            );
            signatures.push({
                signature: await wallet.signMessage(ethers.utils.arrayify(payload)),
                deadline,
            });
        ...
        it('executeTransaction should be successful with 3/3 feds require 3', async function() {
            ...
            const deadline = createTimestamp(2);
            const signatures = [];
            let wallet = new ethers.Wallet(fedMember2Pk);
            const payload = ethers.utils.solidityPack(
                ['bytes32', 'uint256', 'address', 'uint256'],
                [transactionId, 1, this.federation.address, deadline],
            );
            ...
            wallet = new ethers.Wallet(fedMember3Pk);
            signatures.push({
                signature: await wallet.signMessage(ethers.utils.arrayify(payload)),
                deadline,
            });
        ...
        it('should fail if not federation member', async function() {
            ...
            const signatures = [];
            signatures.push(web3.eth.accounts.sign(transactionId, fedMember2Pk).signature);
        ...
        it('executeTransaction should fail with bad chain id', async function() {
            ...
            const payload = ethers.utils.solidityPack(
                ['bytes32', 'uint256', 'address', 'uint256'],
                [transactionId, 33333, this.federation.address, deadline],
            );
            signatures.push({
                signature: await wallet.signMessage(ethers.utils.arrayify(payload)),
                deadline,
        ...
        it('executeTransaction should fail with bad deadline', async function() {
            ...
            const deadline = createTimestamp(-20);
            const signatures = [];
            const wallet = new ethers.Wallet(fedMember2Pk);
            const payload = ethers.utils.solidityPack(
                ['bytes32', 'uint256', 'address', 'uint256'],
        ...
        it('should fail it one signature is from sender', async function() {
            ...
            const signatures = [];
            signatures.push(web3.eth.accounts.sign(transactionId, fedMember1Pk).signature);
            signatures.push(web3.eth.accounts.sign(transactionId, fedMember2Pk).signature);
